...
def _handle_denylist_config_lite():
    """Reads the denylist configuration and provides feedback to the
    caller when this option has been invoked but a denylist isn't
    available.
...
def analysis_from_csv_lite(format_report, analyze, denylist=None):
    denylist = _handle_denylist_config_lite()


    return analysis_from_csv(format_report=format_report, analyze=True, denylist=denylist, rogues=None, heroes=None)
